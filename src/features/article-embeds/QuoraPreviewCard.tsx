...
function isQuora(url: string) {
  try { return new URL(url).hostname.includes("quora.com") || new URL(url).hostname === "qr.ae"; }
  catch { return false; }
}

// Normalize to a canonical, readable Quora URL and strip trackers.
// - keep www.quora.com paths
// - expand qr.ae by following the redirect via mirror
...
// Try to get the **first real content image** from the HTML body.
// We look for <img ...> and also common lazy attributes.
...
export function QuoraPreviewCard({ url }: { url: string }) {
  ...
  useEffect(() => {
    let dead = false;
    if (!isQuora(url)) { setFail(true); return; }
  ...
  if (fail && !data) {
    return (
      <div className="rounded-2xl border p-4 text-sm">
        ...
        <a href={url} target="_blank" rel="noreferrer" className="text-primary underline">Read on Quora</a>
      </div>
    );
  }
