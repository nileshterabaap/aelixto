...
BEGIN
  ...
  -- Mark posts the client says it has already shown the user as seen.
  ...
  -- First: surface posts created after the current top, bypassing seen filter
  -- so freshly-marked seen posts above can't hide a brand-new post.
  ...
  -- Then: top up from the regular following feed (which filters seen posts).
  RETURN QUERY
  SELECT *
