...
AS $function$
  ...
  -- Bucket by minute so chronological order is preserved at meaningful scales,
  -- then within the same minute interleave by a stable hash of the post id.
  -- This breaks up user/platform clusters that come from bulk imports where
  -- many posts share near-identical created_at timestamps.
