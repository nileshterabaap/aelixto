...
BEGIN
  ...
  -- Mark currently-visible posts as seen so refresh always advances past them.
  ...
  -- Step 1: return items whose feed sort time is newer than the client's top.
  -- Bypasses the seen filter so a fresh post is never hidden by the marking above.
