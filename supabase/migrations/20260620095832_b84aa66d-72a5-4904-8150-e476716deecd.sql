-- Restore get_following_feed_v2 to its pre-12 Jun 16:16 body (filter seen posts inline)
...
-- Remove refresh_following_feed_v2 (it was added after 12 Jun 16:16 and the client
-- will now refresh by re-calling get_following_feed_v2 from scratch).
DROP FUNCTION IF EXISTS public.refresh_following_feed_v2(integer, uuid[], timestamp with time zone);
DROP FUNCTION IF EXISTS public.refresh_following_feed_v2(integer, text[], timestamp with time zone);
DROP FUNCTION IF EXISTS public.refresh_following_feed_v2(integer, uuid[], text);
DROP FUNCTION IF EXISTS public.refresh_following_feed_v2(integer, text[], text);