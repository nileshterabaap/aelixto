...
AS $function$
  WITH cursor_values AS (
    SELECT
      CASE WHEN cursor_key IS NULL THEN NULL ELSE (cursor_key::jsonb->>'recency_band')::integer END AS recency_band,
  ...
  ranked_posts AS (
    SELECT *,
      ntile(20) OVER (ORDER BY sort_time DESC, id DESC) AS recency_band,
      row_number() OVER (
  ...
  WHERE cursor_key IS NULL
    OR rp.recency_band > cv.recency_band
    OR (rp.recency_band = cv.recency_band AND rp.cluster_rank > cv.cluster_rank)
    OR (rp.recency_band = cv.recency_band AND rp.cluster_rank = cv.cluster_rank AND rp.shuffle_key < cv.shuffle_key)
    OR (rp.recency_band = cv.recency_band AND rp.cluster_rank = cv.cluster_rank AND rp.shuffle_key = cv.shuffle_key AND rp.sort_time < cv.sort_time)
    OR (rp.recency_band = cv.recency_band AND rp.cluster_rank = cv.cluster_rank AND rp.shuffle_key = cv.shuffle_key AND rp.sort_time = cv.sort_time AND rp.id < cv.id)
  ORDER BY rp.recency_band ASC,
           rp.cluster_rank ASC,
           rp.shuffle_key DESC,
           rp.sort_time DESC,
           rp.id DESC
