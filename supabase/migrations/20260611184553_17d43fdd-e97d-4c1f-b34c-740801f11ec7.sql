...
AS $function$
  ...
  -- Freshness tiers: 0 = <1h, 1 = <6h, 2 = <24h, 3 = <3d, 4 = <7d, 5 = older
  -- Within each tier, posts are shuffled (deterministic per viewer+post)
  -- and interleaved across authors+platforms via cluster_rank.
  tiered AS (
    SELECT
      e.*,
      CASE
        ...
        ELSE 5
      END AS tier,
      -- Deterministic per-viewer shuffle score in [0,1)
