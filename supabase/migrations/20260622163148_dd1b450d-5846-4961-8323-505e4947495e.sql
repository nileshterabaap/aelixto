CREATE OR REPLACE FUNCTION public.get_following_feed_v3(
  limit_count integer,
  cursor_key text DEFAULT NULL::text
)
RETURNS TABLE(
  id uuid,
  user_id uuid,
...
BEGIN
  ...
  WITH base AS (
    SELECT *
    FROM public.get_following_feed_v3(cap, NULL::text)
  ),
  ordered AS (
    SELECT
      b.*,
...
REVOKE EXECUTE ON FUNCTION public.get_following_feed_v3(integer, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_following_feed_v3(integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_following_feed_v3(integer, text) TO service_role;
