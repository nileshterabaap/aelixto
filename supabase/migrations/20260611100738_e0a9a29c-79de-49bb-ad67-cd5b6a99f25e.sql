-- Replace get_following_feed_v2 with a freshness-first, interleaved variant.
-- Goals:
--  1. Strict freshness priority (hour-bucketed)
--  2. Within each hour bucket, interleave by (user, platform) so the same
--     author/platform never stacks consecutively
--  3. Deterministic cursor pagination so every eligible post is delivered
--     exactly once before the feed ends.
...
AS $function$
  WITH cursor_values AS (
    SELECT
      CASE WHEN cursor_key IS NULL THEN NULL ELSE ((cursor_key::jsonb)->>'bucket')::timestamptz END AS c_bucket,
  ...
  -- Hour-bucket gives a freshness band; within a band, cluster_rank
  -- interleaves so the Nth post from each (user, platform) appears together.
  -- That naturally spreads different followings and platforms apart while
  -- preserving newest-first across bands.
  ranked AS (
    SELECT
      e.*,
  ...
  WHERE (
    cursor_key IS NULL
    OR r.bucket < cv.c_bucket
    OR (r.bucket = cv.c_bucket AND r.cluster_rank > cv.c_rank)
    OR (r.bucket = cv.c_bucket AND r.cluster_rank = cv.c_rank AND r.sort_time < cv.c_sort_time)
    OR (r.bucket = cv.c_bucket AND r.cluster_rank = cv.c_rank AND r.sort_time = cv.c_sort_time AND r.id < cv.c_id)
  )
