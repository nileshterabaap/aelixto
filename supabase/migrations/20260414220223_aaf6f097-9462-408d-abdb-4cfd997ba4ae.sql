...
-- Create post_seen table
CREATE TABLE public.post_seen (
...
CREATE INDEX idx_post_seen_user ON public.post_seen (user_id);
...
CREATE POLICY "Users can view their own seen records"
  ON public.post_seen FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can mark posts as seen"
  ON public.post_seen FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can clear their seen history"
  ON public.post_seen FOR DELETE
  USING (auth.uid() = user_id);

-- Update get_following_feed to exclude seen posts
