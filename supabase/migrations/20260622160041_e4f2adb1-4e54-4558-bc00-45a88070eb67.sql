...
CREATE OR REPLACE FUNCTION public.get_following_feed_v2(limit_count integer, cursor_key text DEFAULT NULL::text, refresh_seed text DEFAULT '')
  ...
  tiered AS (
    SELECT
      ...
      CASE
        ...
        ELSE 5
      END AS tier,
      (abs(hashtext(coalesce(refresh_seed,'') || ':' || coalesce(auth.uid()::text,'') || ':' || e.id::text))::double precision
