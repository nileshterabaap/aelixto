...
CREATE OR REPLACE FUNCTION public.get_following_feed_v2(
  limit_count integer,
...
RETURNS TABLE(
  ...
  reposted_at timestamp with time zone, feed_cursor text
)
LANGUAGE sql
...
AS $function$
  ...
  -- new-follow window: accounts followed in the last 7 days
  new_follows AS (
    SELECT following_id
    FROM public.follows
  ...
  tiered AS (
    SELECT *,
      CASE
        WHEN sort_time > now() - interval '24 hours' THEN 1   -- latest unseen
        WHEN from_new_follow THEN 2                            -- new-follow boost
        ELSE 3                                                 -- older unseen
      END AS tier
    FROM eligible_posts
  )
  SELECT t.id, t.user_id, t.content, t.created_at, t.likes_count, t.saves_count,
         ...
         t.preview_image_url, t.is_public, t.profile_id, t.profile_username,
